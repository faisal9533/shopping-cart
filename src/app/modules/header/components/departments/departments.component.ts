...
export class DepartmentsComponent implements OnInit, OnDestroy, AfterViewInit, AfterViewChecked {
    ...
    ngOnInit(): void {
        ...
        this.fetchCategories().subscribe(response => {
            if (response && response.data) {
                let menus = this.updateResponseFormat(response.data);
                this.items = menus;
            }
        });
                ...
                category.LstCategoryDetail.forEach((subCategory: any) => {
                    ...
                    if (subCategory.LstProduct) {
                        subCategory.LstProduct.forEach((product: any) => {
                            let productSlug = product.ProductName.toLowerCase().replace(/ /g, "");
                            products.push({ label: product.ProductName,url: `/shop/products/${productSlug}`, ...product })
                        });
                    }
                    let subCategorySlug = subCategory.SubCategoryName.toLowerCase().replace(/ /g, "");
                    subCategories.push(
                        {
                            size: 6,
                            items: [{
                                label: subCategory.SubCategoryName,
                                url: `/shop/catalog/${catslug}/${subCategorySlug}`,
                                // url: `javascript:void(0)`,
                                items: products, ...subCategory
                            }]
                        });
                });
            }
    ...
    fetchCategories(): Observable<any> {
        return this.http.post(
            `${environment.apiUrl}/POS/ProductCatAndSubCatGet`, JSON.stringify({}))
            .pipe(
                tap((response) => {
                })
            );
