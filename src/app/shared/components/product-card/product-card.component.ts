...
export class ProductCardComponent implements OnInit, OnDestroy, OnChanges {
    ...
    ngOnInit(): void {
      //  console.warn('my product block 1978 ');
  
        this.availability=this.product.availability =='out-of-stock'?false:true;
        console.log(this.product.availability + ' '+this.product.name) ;
        if(this.product && this.product.cartQty){  
            this.quantity.setValue(this.product.cartQty==""?0:this.product.cartQty)
        }
    ...
    ngOnChanges(changes: SimpleChanges): void {
        if ('product' in changes) {
            this.featuredAttributes = !this.product ? [] : this.product.attributes.filter((x:any) => x.featured);
        }
    }
    ...
    addToCart(): void {
        ...
        let quantity = this.quantity.value ? this.quantity.value : 1;
        this.quantity.setValue(quantity);
        this.cart.add(this.product, quantity,[],false).subscribe({
            complete: () => {
